...
Scalability and reliability doesn't make a difference between all these products, except for MQTT, as all of them are highly scalable, robust and reliable. With regards to performance, it's really difficult to evaluate objectively even with standardized benchmarks. However, as already stated, ZeroMQ is by far the most performant message queue when message persistence is not required. Althought there's no much difference between other solutions, RabbitMQ tends to beat others when using AMQP protocol.
